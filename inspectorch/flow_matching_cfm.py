...
class FlowMatchingCFMBackend:
    ...
    def __init__(self, sigma: float = 0.0, method: str = "independent", **kwargs):
        ...
        if method == "exact":
            if ExactOptimalTransportConditionalFlowMatcher is not None:
                ...
                print(
                    "WARNING: Exact OT matcher unavailable (missing pot?). Falling back to Independent CFM."
                )
            ...
            if TargetConditionalFlowMatcher is not None:
                ...
                print(
                    "WARNING: Target matcher unavailable. Falling back to Independent CFM."
                )
                self.matcher = ConditionalFlowMatcher(sigma=sigma)
        elif method == "sb":
            if SchrodingerBridgeConditionalFlowMatcher is not None:
                ...
                print(
                    "WARNING: SB matcher unavailable. Falling back to Independent CFM."
                )
                self.matcher = ConditionalFlowMatcher(sigma=sigma)
        else:
            # Default independent
        ...
        **kwargs,
        ...
        if architecture == "AdaMLP":
            if VectorFieldAdaMLP is not None:
                ...
                print(
                    "WARNING: VectorFieldAdaMLP not available. Using simple MLP fallback."
                )
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for batch in pbar:
                ...
                if self.method in ["exact", "sb"]:
                    ...
                    t, xt, ut = self.matcher.sample_location_and_conditional_flow(
                        x0, x1
                    )
                else:
                    t, xt, ut = self.matcher.sample_location_and_conditional_flow(
    ...
    def check_variables(
        ...
        """
        Checks latent space by transforming samples backwards to noise from t=1
        (Data) to t=0 (Noise).
        """
        ...
        if hasattr(train_loader.dataset, "patches"):
            ...
            print(
                "Warning: Dataset format not recognized for random sampling. Using sequential loader."
            )
            data_source = None
        ...
        with torch.no_grad():
            if data_source is not None:
                random_indices = np.random.randint(
                    0, len(data_source), size=total_samples
                )
    ...
    def log_prob(
        ...
        **kwargs,
        ...
        """
        Compute log-probability using Continuous Normalizing Flow trace
        estimation.
        ...
        """
        if use_naive:
            return self._log_prob_naive(
                inputs, solver_method, step_size, device, batch_size, **kwargs
            )
        else:
            return self._log_prob_api(
                inputs, solver_method, step_size, device, batch_size, **kwargs
            )
    ...
    def _log_prob_naive(
        ...
        **kwargs,
        ...
        Computes log-probability by integrating ODE backward from Data
        (t=1) to Noise (t=0).
        """
        from tqdm import tqdm
    ...
    def _log_prob_api(
        ...
        **kwargs,
        ...
        if solver_method in ["euler", "rk4", "midpoint"]:
            ...
            kwargs.get("step_size", step_size)  # Check kwargs or argument
            ...
            if "step_size" in kwargs:  # Explicit kwarg
                ...
                step_size != 0.1
            ):  # Explicit arg? (hard to tell if default, assume used if changed)
                computed_steps = int(1.0 / step_size)
                t_span = torch.linspace(1, 0, computed_steps, device=device)
            else:
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="Computing log prob"):
                    ...
                    def forward(self, t, state):
                        ...
                        with torch.enable_grad():
                            ...
                            grad_v_eps = torch.autograd.grad(
                                v_eps, x_curr, create_graph=False
                            )[0]
                            div = torch.sum(grad_v_eps * epsilon, dim=-1)
                ...
                state0 = torch.cat(
                    [x, torch.zeros(x.shape[0], 1, device=device)], dim=-1
                )
                ode = ODEFunc(self.velocity_model)

                # Integrate (no options needed when using linspace t_span)
                traj = odeint(
                    ode, state0, t_span, method=solver_method, atol=atol, rtol=rtol
                )
