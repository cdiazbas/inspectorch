...
class VelocityResNetFlow(nn.Module):
    ...
    This directly uses the same ResidualNet architecture from nflows
    that is used in normalizing flows, but adapted for flow matching
    with time conditioning. This ensures a fair comparison between
