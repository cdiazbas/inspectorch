...
This test verifies that the MPS workaround for torchdiffeq is properly
applied. It checks that float32 is automatically injected when running
on MPS devices.
"""
...
def test_mps_detection_cpu():
    ...
    This test runs a simple ODE integration on CPU and verifies it
    works.
    """
    ...
    assert solution[-1].item() < solution[0].item(), (
