...
class MockDataset(torch.utils.data.Dataset):
    ...
    FlowMatchingBackend expects .y_mean, .y_std, and .patches
    attributes.
    """
    ...
    def __init__(self, data_tensor):
        ...
        # Avoid zero std
        self.y_std = torch.where(
            self.y_std < 1e-6, torch.ones_like(self.y_std), self.y_std
        )
...
def test_backend(backend_name, data, sequence_length=1):
    ...
    try:
        ...
        model.train_flow(
            train_loader=loader, num_epochs=epochs, learning_rate=lr, save_model=False
        )
        end_time = time.time()
    ...
    try:
        start_time = time.time()
        lp_trained = model.log_prob(
            SubsetWrapper(sample_subset), batch_size=10, **lp_kwargs
        )
        end_time = time.time()
    ...
    class GridWrapper:
        def __init__(self, g):
            ...
            self.y_mean = (
                dataset.y_mean if hasattr(dataset, "y_mean") else torch.zeros(2)
            )
    ...
    try:
        ...
        lp_grid = model.log_prob(
            GridWrapper(grid), batch_size=400, **lp_kwargs
        ).reshape(20, 20)
        plot_results(
            backend_name,
            train_data,
