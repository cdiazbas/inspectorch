...
model = flowutils.create_flow(input_size=CHANNELS, num_layers=20, hidden_features=64,num_bins=8)

args.learning_rate = 5e-4 
args.num_epochs = 10

dict_info = flowutils.train_flow(model, train_loader, learning_rate=args.learning_rate, num_epochs=args.num_epochs, device=device)

flowutils.plot_train_loss(dict_info['train_loss_avg'])


# Evaluate all the dataset and get the log probability of each point
log_prob = model.log_prob(inputs=(torch.from_numpy(data) - y_mean)/y_std).detach().numpy()
